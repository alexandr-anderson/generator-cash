...
### Смена провайдера модели — это только секреты

Ни текстовый провайдер, ни картиночный не зашиты в код. Чтобы переехать, достаточно поменять секреты и запустить деплой — по три на каждую сторону, стороны независимы:

| Текст | Картинки |
|-------|----------|
| `OPENAI_BASE_URL` | `OPENAI_IMAGE_BASE_URL` |
| `OPENAI_API_KEY` | `OPENAI_IMAGE_API_KEY` |
| `OPENAI_MODEL` | `OPENAI_IMAGE_MODEL` |

**Имя модели ни на что в коде не влияет** (с 2026-09-11). Раньше форму запроса выбирала эвристика по имени (`/^gpt-5/`), и достаточно было назвать модель иначе, чтобы ей молча ушёл не тот набор параметров — наша собственная `chatgpt-5.6` под тот шаблон уже не подходила. Теперь код спрашивает у самого шлюза: отправляет современную форму (`max_completion_tokens`, без `temperature`), а на `400` переходит к прежней (`max_tokens` + `temperature`), затем к вариантам без JSON-режима. Каждый `400` возвращается мгновенно, так что перебор почти ничего не стоит.

Прочее тоже сделано терпимо к чужим шлюзам:
- ответ картинки понимаем и как `b64_json`/`b64`/`base64`, и по ссылке; список берём из `data` или `images`;
- у картинок две формы тела запроса, вторая идёт при `400`;
...
**Запасные шлюзы картинок** — после `OPENAI_IMAGE_*` картинки поста и обложки идут по шлюзам из [`src/lib/image-providers.ts`](../src/lib/image-providers.ts): Pollinations `z-image-turbo` (рисует по сцене от текстовой модели, по очереди), затем codex.sale `gpt-image-2` (обычный промпт, все оставшиеся картинки одновременно — он рисует 1,5–3 минуты на картинку). Секреты — только ключи: `POLLINATIONS_API_KEY` (тот же, что у текстовой цепочки), `CODEX_SALE_API_KEY`. В стартовом логе `imageFallback=pollinations,codex-sale`. Прежние `IMAGE_FALLBACK_*` больше не читаются. Подробности — п. 50 в [work-plan.md](work-plan.md).
...
`OPENAI_BASE_URL`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `OPENAI_IMAGE_BASE_URL`, `OPENAI_IMAGE_API_KEY`, `OPENAI_IMAGE_MODEL`, `ZAI_API_KEY`, `POLLINATIONS_API_KEY`, `CODEX_SALE_API_KEY` (пустые не пишутся), `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT`, `TELEGRAM_CHAT_ID`.
...
### Вторая причина: аккаунту не хватает процессов

Следующий же деплой упал иначе — `exit 134` и `Assertion failed: uv_thread_create` при запуске
node. Это тот же лимит шейред-хостинга, из-за которого на сервере нельзя собирать Next.js. Крэш
случился **до** вызова PM2, то есть проверка pid даже не успела отработать.

Что сделано:
- **бэкап и миграции больше не идут на каждом рестарте.** Сначала `apply-migrations.php --check`
  (дёшево, PHP) отвечает кодом выхода, есть ли неприменённые миграции: `10` — есть, `0` — нет.
  Нет — пропускаем и бэкап, и применение. Бэкап нужен перед изменением схемы, а не перед каждым
  перезапуском, и раньше он занимал процессы ровно в тот момент, когда они нужны PM2;
- **вызовы PM2 повторяются трижды при коде 134** с нарастающей паузой. Нехватка процессов —
  состояние временное. Другие коды ошибок не повторяются: это настоящие проблемы, а не фон.

Если и три попытки не прошли — на аккаунте действительно кончились процессы. Проверьте, не висят ли
лишние (`ps ux`), и повторите позже.
