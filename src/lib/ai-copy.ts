...
export async function composeExpertCopy(input: {
  ...
  const payload = await openaiJson<Record<string, unknown>>({
    ...
    user: [
      ...
      `Тон: ${input.tone || "спокойный и уверенный"}`,
      source
        ? `Исходный текст автора (не противоречь смыслу, можно сжать и разложить по слайдам):\n${source}`
        : "Исходного текста нет — напиши экспертный текст сам и разложи его.",
      "",
      "Нужно:",
      "- text: один общий текст (если автор дал свой — верни его слегка вычитанным, не переписывай с нуля)",
      `- caption: подпись к публикации, отдельно от слайдов. Для поста это развёрнутый текст. Для карусели — хук + «листайте» + вывод. Для обложки Reels — подпись к ролику.`,
      "- hashtags: массив из 10–15 хештегов на русском и латинице, без пробелов внутри",
      input.format === "reel"
        ? "- reelScript: хук и 4–6 тезисов, что говорить в ролике"
        : "- reelScript можно опустить",
      `- scenarios: ровно 3 объекта. У каждого name из списка ниже и slides: ровно ${slideCount} строк.`,
      "Текст на трёх сценариях один по смыслу, меняется только драматургия. Не три пересказа разными словами ради объёма.",
